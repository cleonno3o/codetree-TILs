...
void shootBall()
{
	...
	if (isFound)
	{
		...
		teams[targetTeam].head = headBack;
		teamBoard[teams[targetTeam].head.first][teams[targetTeam].head.second] = 1;
		teamBoard[teams[targetTeam].tail.first][teams[targetTeam].tail.second] = 3;
	}
	offset++;
	if (offset == n)
	{
